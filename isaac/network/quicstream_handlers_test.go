...
func (t *testQuicstreamHandlers) TestStartHandover() {
	...
	t.Run("error", func() {
		...
		t.Run("not matched local node", func() {
			...
			t.Error(err)
			t.False(started)
			t.ErrorContains(err, "node address not matched")
		})

		t.Run("same node conn info", func() {
			openstreamf, handlercancel := testOpenstreamf(t.Encs, HandlerPrefixStartHandover, handler)
			defer handlercancel()
			...
			t.Error(err)
			t.False(started)
			t.ErrorContains(err, "same node conn info")
		})
	...
	t.Run("ok", func() {
		...
		handler := QuicstreamHandlerStartHandover(
			...
			t.LocalParams.NetworkID(),
			yci,
			func(base.Address, quicstream.UDPConnInfo) error {
				startedch <- struct{}{}
